...
def test_good_cheese_list_view_expanded(rf):
    ...
    #   HTTP responsme served up by Django
    response = callable_obj(request)
    # Test that the HTTP response has 'Cheese List' in the
    #   HTTP and has a 200 response code
    assertContains(response, 'Cheese List')

def test_good_cheese_view(rf):
    # Get the request
...
def test_cheese_create_correct_title(rf, admin_user):
    """Page title for CheeseCrateView should be Add Cheese"""
    request = rf.get(reverse('cheeses:add'))
    request.user = admin_user
    response = CheeseCreateView.as_view()(request)
...
def test_good_cheese_update_view(rf, admin_user, cheese):
    ...
    # Make a request for our new cheese
    request = rf.get(url)
    # Ad an authenticated user
    request.user = admin_user
    # Use the rquest to get the response
